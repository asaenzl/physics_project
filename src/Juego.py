...
# creamos la figura
plt.figure("FISICA APLICADA", figsize=(10, 8), dpi=80, facecolor="#038554", edgecolor="c")
plt.axes()
# añadimos el titulo
title("LANZAMIENTO DE PROYECTILES",
      fontsize=15, color="black", verticalalignment="baseline", horizontalalignment="center")
# añadimos el subtitulo
plt.suptitle("PROYECTO FISICA 1", fontsize=20, color="black")

# añadimos las etiquetas de los ejes
xlabel("xmax", fontsize=20, color="black")
ylabel("ymax", fontsize=20, color="black")
# añadimos texto
...
plt.text(2, np.max(f(x)) + 1, ("fecha/hora:" + str(time.asctime())), fontsize=10, color="green")
plt.text(2, np.max(f(x)) - 1, ("tiempo(s):" + str(time.process_time())), fontsize=10, color="blue")
