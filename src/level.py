from personajes import Objetivo
...
class Level():
    ...
    def open_flat(self, x, y, n):
        ...
        for i in range(n):
            y = y0 + 100 + i * 100
            p = (x, y)
            self.columns.append(Poligono(p, 20, 85, self.space))
            p = (x + 60, y)
            self.columns.append(Poligono(p, 20, 85, self.space))
            p = (x + 30, y + 50)
            self.beams.append(Poligono(p, 85, 20, self.space))
    ...
    def build_0(self):
        """level 0"""
        pig1 = Objetivo(980, 100, self.space)
        pig2 = Objetivo(985, 182, self.space)
